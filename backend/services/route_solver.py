...
def _latest_feasible_departure(
    ...
    *,
    ...
    if first_arrival > deadline:
        # Leaving as early as possible still misses the deadline, usually
        # because the deadline is nearer than the journey is long. Refusing to
        # plan at all is the wrong answer: the journey is still drivable and
        ...
        first["schedule_feasibility"] = {
            ...
            "earliest_arrival": clock.iso(first_arrival),
            "shortfall_mins": round(
                (first_arrival - deadline).total_seconds() / 60.0, 1,
            ),
            "note": (
                "The requested arrival cannot be met. This plan departs at the "
                "earliest possible time and arrives "
                f"{round((first_arrival - deadline).total_seconds() / 60.0)} "
    ...
    best["schedule_feasibility"] = {
        ...
        "earliest_departure": clock.iso(low),
        "earliest_arrival": best.get("estimated_arrival"),
        "shortfall_mins": 0.0,
        "note": None,
    }
    return best
...
def _multi_stop_alternative_journeys(
    *,
    ...
    """Build whole-journey alternatives by varying the journey's longest legs.

    Solving every leg with alternatives multiplies out: three options across
    four legs is eighty-one journeys, each leg a full A* over a 115k-node
    graph. Varying the longest legs keeps the cost to a bounded number of
    extra searches while changing the parts of the journey where a different
    path actually moves the total.
    ...
    # Longest first, because that is where a different path moves the total
    # most. One leg often yields only a single distinct alternative, so fall
    # through to the next-longest rather than returning fewer options than
    # asked for. Bounded so the extra searches stay predictable.
    candidate_legs = sorted(
        road_legs, key=lambda index: legs[index]["distance_km"], reverse=True,
    )[:limit]

    options: List[Dict[str, Any]] = []
    failure_note: Optional[str] = None

    for varied_index in candidate_legs:
        ...
        try:
            ...
            # primary chain is already solved and remains fully usable.
            failure_note = (
        ...
        for candidate in varied.get("alternative_routes") or []:
            if len(options) >= limit:
                break
            ...
            for index, leg in enumerate(legs):
                ...
                if index == varied_index:
                    ...
                    # same delta. Times move; the modelled congestion behind
                    # them does not, which each option declares.
            ...
            options.append({
                ...
                "description": (
                    "Same stops in the same order, taking a different road "
                    f"path between {start['name']} and {end['name']}."
                ),
    ...
    if not options:
        return [], failure_note or (
            "The road engine found no distinct alternative for this "
            "journey's road legs, so only the primary chain is offered."
        )
    return options, None
